...
loop(State) ->
   ...
   {mysql_recv, RecvPid, closed, normal} ->
	    ?Log(LogFun, error, "socket closed by server"),
       error;
   {mysql_recv, RecvPid, closed, {error, Reason}} ->
	    ?Log2(LogFun, error,
          "socket closed by server. Error = ~p", [Reason]),
       error;
   Unknown ->
	    ?Log2(LogFun, error,
		  "received unknown signal, exiting: ~p", [Unknown]),
...
get_query_response(LogFun, RecvPid, Version) ->
   case get_query_response_aux(LogFun, RecvPid, Version) of
      [Single] ->
         Single;
      List ->
         List
   end.
get_query_response_aux(LogFun, RecvPid, Version) ->
   case do_recv(LogFun, RecvPid, undefined) of
   {ok, <<Fieldcount:8, Rest/binary>>, _} ->
	   case Fieldcount of
		0 ->
         {AffectedRows, Rest1} = get_binary(Rest),
         {_InsertId, Rest2}    = get_binary(Rest1),
         {ServerStatus, _Rest3} = get_binary(Rest2),
         case ServerStatus band ?SERVER_MORE_RESULTS of
            ?SERVER_MORE_RESULTS ->
               [{updated, #mysql_result{affectedrows=AffectedRows}} | get_query_response_aux(LogFun, RecvPid, Version)];
            0 -> %% No Tabular data
			   ...
			   case get_rows(Fields, LogFun, RecvPid, []) of
				{ok, Rows} ->
				    [{data, #mysql_result{fieldinfo=Fields,
                         rows=Rows}}];
            {ok, Rows, more_results} ->
				    [{data, #mysql_result{fieldinfo=Fields,
                         rows=Rows}} | get_query_response_aux(LogFun, RecvPid, Version)];
				{error, Reason} ->
   ...
   case do_recv(LogFun, RecvPid, undefined) of
	{ok, Packet, _Num} ->
		...
		<<254:8, Rest/binary>> when size(Rest) < 8 ->
		    {ok, lists:reverse(Res)};
		_ ->
		    {Table, Rest} = get_string(Packet),
		    {Field, Rest2} = get_string(Rest),
		    {Length, Rest3} = get_string(Rest2),
		    {Type, Rest4} = get_string(Rest3),
		    {_Flags, _Rest5} = get_string(Rest4),
		    This = {Table,
			    Field,
			    Length,
	...
	{ok, Packet, _Num} ->
	   case Packet of
		...
		<<254:8, Rest/binary>> when size(Rest) < 8 ->
		    {ok, lists:reverse(Res)};
		_ ->
		    {_Catalog, Rest} = get_string(Packet),
		    {_Database, Rest2} = get_string(Rest),
		    {Table, Rest3} = get_string(Rest2),
		    %% OrgTable is the real table name if Table is an alias
		    {_OrgTable, Rest4} = get_string(Rest3),
		    {Field, Rest5} = get_string(Rest4),
		    %% OrgField is the real field name if Field is an alias
		    {_OrgField, Rest6} = get_string(Rest5),
...
get_rows(Fields, LogFun, RecvPid, Res) ->
   case do_recv(LogFun, RecvPid, undefined) of
	{ok, Packet, _Num} ->
	   case Packet of
      <<254:8, _Warnings:16, ServerStatus:16/little>> when ServerStatus band ?SERVER_MORE_RESULTS == ?SERVER_MORE_RESULTS->
         {ok, lists:reverse(Res), more_results};
		<<254:8, Rest/binary>> when size(Rest) < 8 ->
		    {ok, lists:reverse(Res)};
		_ ->
...
get_binary(<<251:8, Rest/binary>>) ->
   {null, Rest};
get_binary(<<252:8, Length:16/little, Rest/binary>>) ->
   {Length, Rest};
get_binary(<<253:8, Length:24/little, Rest/binary>>) ->
   {Length, Rest};
get_binary(<<254:8, Length:64/little, Rest/binary>>) ->
   {Length, Rest};
get_binary(<<Length:8, Rest/binary>>) when Length < 251 ->
   {Length, Rest}.
